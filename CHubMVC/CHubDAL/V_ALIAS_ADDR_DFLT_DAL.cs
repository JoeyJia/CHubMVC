...
namespace CHubDAL
{
    public class V_ALIAS_ADDR_DFLT_DAL : BaseDAL
    {
        ...
        public List<V_ALIAS_ADDR_DFLT> GetStrictAliasAddrDFLT(string shipName, string addr)
        {
            return db.V_ALIAS_ADDR_DFLT.Where(a => a.LOCAL_SHIP_TO_NAME == shipName && a.LOCAL_SHIP_TO_ADDR_1 == addr && a.ACTIVEIND == CHubConstValues.IndY).OrderBy(a => a.DAYS).ToList();
        }
